...
function renderWorldCup(fc){
  ...
  // Phase Finale — Bracket avec drapeaux
  ...
  function vct(){return'<div class="bm"><div class="bt" style="color:rgba(0,0,0,.25);font-style:italic">Vainqueur</div></div>';}
  function flt(){return'<div class="bm"><div class="bt" style="color:#0071e3;font-weight:700">Finaliste</div></div>';}
  
  h+='<div class="bwrap">';
  // Moitié Haute
  h+='<div class="bhalf"><div class="bhlbl">🔼 Moitié Haute</div>';
  h+='<div class="brow">'+bcol('16es',[bm(gt('A',0),gt('B',1)),bm(gt('C',0),gt('D',1))])+barr()+bcol('8es',[vct()])+barr()+bcol('Quart',[vct()])+barr()+bcol('Demi',[flt()])+'</div>';
  h+='<div class="brow" style="margin-top:2px">'+bcol('',[bm(gt('E',0),gt('F',1)),bm(gt('G',0),gt('H',1))])+barr()+bcol('',[vct()])+'</div>';
  h+='</div>';
  
  // Moitié Basse
  h+='<div class="bhalf"><div class="bhlbl">🔽 Moitié Basse</div>';
  h+='<div class="brow">'+bcol('16es',[bm(gt('B',0),gt('A',1)),bm(gt('D',0),gt('C',1))])+barr()+bcol('8es',[vct()])+barr()+bcol('Quart',[vct()])+barr()+bcol('Demi',[flt()])+'</div>';
  h+='<div class="brow" style="margin-top:2px">'+bcol('',[bm(gt('F',0),gt('E',1)),bm(gt('H',0),gt('G',1))])+barr()+bcol('',[vct()])+'</div>';
  h+='</div>';
  h+='</div>';
